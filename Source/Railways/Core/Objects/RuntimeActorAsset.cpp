...
//void ARuntimeActorAsset::BuildMeshNode(const AssimpNode& Node)
//{
...
//        }
//
//        DynamicMesh->SetMaterial(MeshIndex, DynMaterial);
//
//        DynamicMesh->CreateMeshSection(MeshIndex, Positions, Triangles, Normals, UVs, Colors, Tangents, false);
...
//void ARuntimeActorAsset::BuildNodeTree(const AssimpNode& Node)
//{
//    FProcSkeletalMeshNode NewNode;
//    NewNode.BoneTransform = Node.Transform;
//    const int32 NumMeshes = Node.Meshes.Num();
//    static int MeshIndex = 0;
//    for (const AssimpMesh* Mesh : Node.Meshes)
//    {
...
//        //DynamicMesh->SetMaterial(MeshIndex, DynMaterial);
//
//        //DynamicMesh->CreateMeshSection(MeshIndex, NewNode, Positions, Triangles, Normals, UVs, Tangents);
//
//        //Parent.ChildNodes.Add(new FProcSkeletalMeshNode(Positions, Triangles, Normals, UVs, Tangents)); //switch to emplace when finalized
//        MeshIndex++;
//    }
//
//    for (const AssimpNode& Child : Node.Children)
//        NewNode.ChildNodes.Add(BuildNodeTree(Child));
//
//    return NewNode;
//}
...
{
    UE_LOG(LogTemp, Error, TEXT("Improperly initialized runtime content."));
    //BuildMeshNode(*MeshData->RootNode);
    //DynamicMesh->RootNode = BuildNodeTree(*MeshData->RootNode);
